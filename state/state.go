...
type State interface {
	...
	Reduce(action action.ActionName, payload interface{}, dispatcher action.Dispatcher) (State, bool)
}
...
func (ss States) Reduce(action action.ActionName, payload interface{}, dispatcher action.Dispatcher) (State, bool) {
	var bigDirty bool
	for key, state := range ss {
		newState, dirty := state.Reduce(action, payload, dispatcher)
		if dirty {
			ss[key] = newState
			bigDirty = true
		}
...
func (ss *SessionState) Reduce(actionName action.ActionName, payload interface{}, _ action.Dispatcher) (State, bool) {
	switch actionName {
...
func (as *AppState) Reduce(action action.ActionName, payload interface{}, dispatcher action.Dispatcher) (State, bool) {
	var (
		state    State
		dirty    bool
		bigDirty bool
	)
	state, dirty = as.Session.Reduce(action, payload, dispatcher)
	if dirty {
		bigDirty = true
		as.Session = state.(*SessionState)
	}
	state, dirty = as.Data.Reduce(action, payload, dispatcher)
	if dirty {
		bigDirty = true
		as.Data = state.(States)
	}
	state, dirty = as.Views.Reduce(action, payload, dispatcher)
	if dirty {
		bigDirty = true
